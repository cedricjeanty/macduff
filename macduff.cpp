...
{
	ColorChecker2 checker2;
	checker2.error = checker->error;
	checker2.size = checker->size;

	for(int y = 0; y < MACBETH_HEIGHT; y++) {
		for(int x = 0; x < MACBETH_WIDTH; x++) {
			CvScalar this_color = cvGet2D(checker->values,y,x);
			CvScalar this_point = cvGet2D(checker->points,y,x);

			checker2.points[y*MACBETH_WIDTH+x].x = this_point.val[0];
			checker2.points[y*MACBETH_WIDTH+x].y = this_point.val[1];

			checker2.values[y*MACBETH_WIDTH+x].b = this_color.val[0];
			checker2.values[y*MACBETH_WIDTH+x].g = this_color.val[1];
			checker2.values[y*MACBETH_WIDTH+x].r = this_color.val[2];
		}
	}
...
IplImage * mat2IplImage(uint8_t *img , int width, int height )
{
	...
	int channels = 3; 
	IplImage *out_img =  cvCreateImageHeader(size, depth,channels); 
	out_img->imageData = (char*)img;
	return out_img;
}
...
ColorChecker find_macbeth(IplImage* macbeth_img)
{
	...
	// int depth  = IPL_DEPTH_8U; 
	// int channels = 3; 
	// IplImage *macbeth_img =  cvCreateImageHeader(size, depth,channels); 
	...
	if( macbeth_img )
	{
		...
		// return macbeth_img;
		// return convert_colorChecker(&found_colorchecker);
		return found_colorchecker;
	}
